...
(async()=>{
 ...
 await authContext.route('https://onecard-test.supabase.co/**',async route=>{
  ...
  if(url.pathname.endsWith('/otp')){otpRequests.push(body);}
 ...
 await auth.click('#account-button');await auth.click('#sign-out');await auth.waitForFunction(()=>document.querySelector('#account-signed-in').hidden);assert.equal(await auth.locator('.loyalty-card').count(),1);assert.match(await auth.locator('.loyalty-card').innerText(),/Guest/);
